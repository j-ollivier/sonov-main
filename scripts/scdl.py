#!/usr/bin/env python3
# -*- encoding: utf-8 -*-
# source : https://github.com/flyingrub/scdl/blob/master/scdl/scdl.py
...
def in_download_archive(track):
    ...
    try:
        with open(archive_filename, 'a+', encoding='utf-8') as file:
            ...
            file.seek(0)
            track_id = '{0}'.format('{}.mp3'.format(track['id']))
            for line in file:
...
def record_download_archive(track):
    ...
    try:
        with open(archive_filename, 'a', encoding='utf-8') as file:
            file.write('{0}'.format('{}.mp3'.format(track['id']))+'\n')
    except IOError as ioe:
